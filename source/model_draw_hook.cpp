...
void SetForceStaticLighting(bool enable) {
    ...
    Msg("[gmRTX - ModelDrawHook] Force static lighting %s\n", enable ? "enabled" : "disabled");
}
...
void SetModelDrawHookEnabled(bool enable) {
    ...
    Msg("[gmRTX - ModelDrawHook] Hook %s\n", enable ? "enabled" : "disabled");
}
    ...
    DrawModelInfo_Internal_t* pInfo)     // The DrawModelInfo structure (rdx in calling convention)
{
    ...
    if (!pInfo || IsBadReadPtr(pInfo, sizeof(DrawModelInfo_Internal_t))) {
        Warning("[gmRTX] Invalid pInfo pointer: 0x%p, calling original function\n", pInfo);
        return StudioRender_DrawModel_trampoline()(_this, pInfo);
    }

    // Additional safety check: ensure we're not getting garbage values
    if ((uintptr_t)pInfo < 0x1000 || (uintptr_t)pInfo > 0x7FFFFFFFFFFF) {
        Warning("[gmRTX] Suspicious pInfo pointer value: 0x%p, calling original function\n", pInfo);
        return StudioRender_DrawModel_trampoline()(_this, pInfo);
    }
    ...
    if (GetForceStaticLighting()) {
        ...
        __try {
            ...
            if (IsBadReadPtr(&pInfo->m_DrawFlags, sizeof(int))) {
                Warning("[gmRTX] Cannot read flags field at offset 0x40, calling original function\n");
                return StudioRender_DrawModel_trampoline()(_this, pInfo);
            }
            
            // Check if we can write to the flags field
            if (IsBadWritePtr(&pInfo->m_DrawFlags, sizeof(int))) {
                Warning("[gmRTX] Cannot write to flags field at offset 0x40, calling original function\n");
                return StudioRender_DrawModel_trampoline()(_this, pInfo);
            }
        ...
        __except(EXCEPTION_EXECUTE_HANDLER) {
            Warning("[gmRTX] Exception accessing pInfo structure, calling original function\n");
            return StudioRender_DrawModel_trampoline()(_this, pInfo);
        }
    }
    ...
    DrawModelResults_t* pResults, const DrawModelInfo_t& info, matrix3x4_t* pBoneToWorld, float* pFlexWeights, float* pFlexDelayedWeights, const Vector& modelOrigin, int flags)
{
    ...
    if (GetForceStaticLighting()) {
        ...
        __except(EXCEPTION_EXECUTE_HANDLER) {
            Warning("[gmRTX] Exception accessing DrawModelInfo_t structure (info=0x%p), calling original function\n", &info);
...
void ModelDrawHook::Initialize() {
    if (m_bInitialized) {
        Msg("[gmRTX - ModelDrawHook] Already initialized, skipping\n");
        return;
    }

    try {
        Msg("[gmRTX - ModelDrawHook] Initializing model draw hooks...\n");
        ...
        if (GlobalConvars::rtx_force_static_lighting) {
            ...
            Msg("[gmRTX - ModelDrawHook] Static lighting synced with ConVar: %s\n", g_forceStaticLighting ? "enabled" : "disabled");
        } else {
            g_forceStaticLighting = true;
            Msg("[gmRTX - ModelDrawHook] Static lighting enabled by default (ConVar not available yet)\n");
        }
        ...
        if (!engineModule) {
            Warning("[gmRTX - ModelDrawHook] Failed to get engine.dll handle\n");
            return;
        }
        Msg("[gmRTX - ModelDrawHook] Found engine.dll at 0x%p\n", engineModule);
        ...
        Msg("[gmRTX - ModelDrawHook] Searching for CStaticProp::DrawModel interface pattern...\n");
        ...
        if (!studioRenderPtrPattern) {
            Warning("[gmRTX - ModelDrawHook] Failed to find CStaticProp::DrawModel interface pattern\n");
            // Try alternative approach - for now just mark as initialized so Lua functions work
            m_bInitialized = true;
            Msg("[gmRTX - ModelDrawHook] Marked as initialized (pattern search failed, but Lua functions will work)\n");
            return;
        }
        Msg("[gmRTX - ModelDrawHook] Found pattern at 0x%p\n", studioRenderPtrPattern);
        ...
        Msg("[gmRTX - ModelDrawHook] Studio render pointer calculated as 0x%p\n", studioRenderPtr);

        if (!studioRenderPtr || !*studioRenderPtr) {
            Warning("[gmRTX - ModelDrawHook] Studio render interface pointer is null\n");
            // Still mark as initialized for Lua functions
            m_bInitialized = true;
            Msg("[gmRTX - ModelDrawHook] Marked as initialized (null pointer, but Lua functions will work)\n");
            return;
        }
        ...
        Msg("[gmRTX - ModelDrawHook] Studio render instance: 0x%p, vtable: 0x%p\n", studioRender, vtable);

        if (!vtable) {
            Warning("[gmRTX - ModelDrawHook] Studio render vtable is null\n");
            // Still mark as initialized for Lua functions
            m_bInitialized = true;
            Msg("[gmRTX - ModelDrawHook] Marked as initialized (null vtable, but Lua functions will work)\n");
            return;
        }
        ...
        if (!drawModelFunc) {
            Warning("[gmRTX - ModelDrawHook] DrawModel function pointer is null\n");
            // Still mark as initialized for Lua functions
            m_bInitialized = true;
            Msg("[gmRTX - ModelDrawHook] Marked as initialized (null function, but Lua functions will work)\n");
            return;
        }

        Msg("[gmRTX - ModelDrawHook] Found static props DrawModel function at 0x%p\n", drawModelFunc);
        ...
        Msg("[gmRTX - ModelDrawHook] Searching for main IStudioRender interface...\n");
        ...
        if (mainStudioRenderPattern) {
            Msg("[gmRTX - ModelDrawHook] Found main IStudioRender pattern at 0x%p\n", mainStudioRenderPattern);
            ...
            if (mainStudioRenderPtr && *mainStudioRenderPtr) {
                ...
                if (mainVtable) {
                    ...
                    if (mainDrawModelFunc) {
                        Msg("[gmRTX - ModelDrawHook] Found dynamic entities DrawModel function at 0x%p\n", mainDrawModelFunc);
                        Setup_Hook(IStudioRender_DrawModel, mainDrawModelFunc);
                    } else {
                        Warning("[gmRTX - ModelDrawHook] Main DrawModel function pointer is null\n");
                    }
                } else {
                    Warning("[gmRTX - ModelDrawHook] Main studio render vtable is null\n");
                }
            } else {
                Warning("[gmRTX - ModelDrawHook] Main studio render interface pointer is null\n");
            }
        } else {
            Warning("[gmRTX - ModelDrawHook] Failed to find main IStudioRender interface pattern\n");
        }

        // Now find and hook the IVModelRender interface for DrawModelExecute (dynamic entities)
        Msg("[gmRTX - ModelDrawHook] Loading IVModelRender interface...\n");
        ...
        if (engineModule2) {
            ...
            if (createEngineInterface) {
                ...
                if (pModelRender) {
                    Msg("[gmRTX - ModelDrawHook] Successfully loaded IVModelRender interface at 0x%p\n", pModelRender);
                    ...
                    if (modelRenderVtable) {
                        ...
                        if (drawModelExecuteFunc) {
                            Msg("[gmRTX - ModelDrawHook] Found DrawModelExecute function at 0x%p\n", drawModelExecuteFunc);
                            Setup_Hook(IVModelRender_DrawModelExecute, drawModelExecuteFunc);
                        } else {
                            Warning("[gmRTX - ModelDrawHook] DrawModelExecute function pointer is null\n");
                        }
                    } else {
                        Warning("[gmRTX - ModelDrawHook] IVModelRender vtable is null\n");
                    }
                } else {
                    Warning("[gmRTX - ModelDrawHook] Failed to load IVModelRender interface\n");
                }
            } else {
                Warning("[gmRTX - ModelDrawHook] Failed to get CreateInterface from engine.dll\n");
            }
        } else {
            Warning("[gmRTX - ModelDrawHook] Failed to get engine.dll handle\n");
        }
        ...
        m_bInitialized = true;
        Msg("[gmRTX - ModelDrawHook] Model draw hooks initialized successfully with static, dynamic, and execute hooks\n");
    }
    catch (const std::exception& e) {
        Warning("[gmRTX - ModelDrawHook] Exception during initialization: %s\n", e.what());
        // Still mark as initialized for Lua functions
        m_bInitialized = true;
        Msg("[gmRTX - ModelDrawHook] Marked as initialized despite exception (Lua functions will work)\n");
    }
    catch (...) {
        Warning("[gmRTX - ModelDrawHook] Unknown exception during initialization\n");
        // Still mark as initialized for Lua functions
        m_bInitialized = true;
        Msg("[gmRTX - ModelDrawHook] Marked as initialized despite unknown exception (Lua functions will work)\n");
    }
}
...
void ModelDrawHook::Shutdown() {
    ...
    try {
        Msg("[gmRTX - ModelDrawHook] Shutting down model draw hooks...\n");
        ...
        Msg("[gmRTX - ModelDrawHook] Model draw hooks shutdown complete\n");
    }
    catch (...) {
        Warning("[gmRTX - ModelDrawHook] Exception during shutdown\n");
    }
} 